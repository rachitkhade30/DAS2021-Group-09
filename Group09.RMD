...
First we perform a visual analysis of the data. Here is part of the data from dataset9. 
```{r head,echo=FALSE}
...
Because the data have many outliers(which is much larger) in the length variables, so we do the log transformation of this variable and then draw a histograms with the log(length).
...
Figure 4 is the relationship between budget and the number of different kinds of films. It's not difficult to see that this is a bell shape, the film whichever the genres it belongs to, the count will first rise and then fall and reach peak at about 12 million dollars.
...
Because the data are located near zero and have many outliers(which is much larger) in the votes variables, so we do the log transformation of this variable and then draw a histograms with the log(votes).
...
So after comparing the Figure 5 using data without transforming into log and the Figure 6 using data with transforming into log, We think the later is better.Figure 6 is the relationship between votes and the number of different kinds of films. As we can see, it is easily to understand that the higher the positive votes a film get, the fewer it is which means only a small number of films can receive a  positive evaluation from the audience. 
...
Figure 7 is a pie plot which shows the proportion of different kinds of films that get a score higher than 7. We can know that the Comedy, Documentary and Animation are the top 3 kinds of films which account for 43.54%, 15.5% and 12.73% respectively.
...
Figure 8 is a pie plot which shows the proportion of different kinds of films that get a score lower than 7. According to the plot we can see that the Drama and Action almost account for 80.52%.
...
So after comparing the Figure 10 using data without transforming into log and the Figure 11 using data with transforming into log, We think the later is better. Figure 11 is a box plot shows the distribution of different kinds of films which have scores lower and upper than 7 of different length. We can easily get the information that the shorter the Drama, Romance and Short films are, the higher their scores are.
...
In figure 12, we can obviously see that higher budget can make a film get higher score. It is easily to understand that an appearance fee for a well-known actor, cost of special effects production and some other reasons are all in the budget. And these elements can all make an influence to the score.
...
boxplot41
```
Because the data  have many outliers(which is much larger) in the votes variables, so we do the log transformation of this variable and then draw a histograms with the log(votes).
...
Figure 15 and Figure 16 are the correlation coefficient between the variables year, length, budget and votes. According to the correlation matrix, we can see that these variables are not very related to each other which means we should not consider the multicollinearity problem in the following data analysis.

\newpage
# Formal Data Analysis {#sec:FDA}
## The Construction of GLM Model {#sec:GLM}
According to previous studies(histogram and boxplot), we find that it is necessary to change votes and length into log(votes) and log(length) to reduce outliers. So we use the log(votes) and log(length) in the Formal Data Analysis analysis.
...
The AIC of model3(with independent variables of log(length), budget and genre) is 1542.3 and according to the stepwise regression, we can know this model is the best one with a lowest AIC. So we choose this moodle.
...
1. For each minute increase in the log(length) of film, the odds of film score decrease (get multiplied by a factor of 0.05);
2. For each $1000000 increase in the budget of film, the odds of film score increase (get multiplied by a factor of 1.71);
3. Animation??? odds of score is 0.07 times those of Action; Comedy??? odds of score is 24.64 times those of Action; Documentary??? odds of score is 174.89 times those of Action; Drama ??? odds of score is 0.17 times those of Action; Short??? odds of score is 79.59 times those of Action;
4. Romance ??? odds of score is same as those of Action(not significant).
...
From the analysis, It can be concluded that factors like length of the movie, budget of the movie received by the movie can highly influence IMDB ratings and can determine which movie might get rated above 7 or less. 
...
Positive votes received by a movie can be a potential feature for forcasting whether or not a movie will gain a rating greater than 7 or not. People tend to consume content which is appealing to everyone. Votes can therefore be an important predictor for IMDB ratings.  

# Future work {#sec:Fut}

Since the movie statistics of IMDB movie database are still missing, and our model has eliminated outliers. In this paper, the rating level of 7 is taken as the standard of good or bad film, but in fact, there is an error in rating. So we left the evaluation of many outliers, the supplement of missing movies, the analysis of multiple databases, and the work of reducing errors to the future. This paper mainly uses a generalized linear model to analyze the data, and future work involves deeper analysis of specific films. In the future work, we mainly have three points:

1. Add rating weights for films from different decades to the model. Because different eras have different cultural styles, a film that is rated less than 7 in one decade could be rated higher than 7 if released in another decade.

2. The total number of movies is missing. In future work, we can try to study several movie databases at the same time and compare and analyze them, so as to avoid a lot of errors.

3. The way to build the model can be changed to more complex logistic regression. And can provide some information analysis about outliers between different images according to different images. In the data that we used as a real example,Analyzing outliers to build models is a tedious task, and no further research can be performed in this direction in the future.

Obviously, in the analysis of films greater than 7, this paper finds that comedies account for 43.54 percent. That's too large a number, so it needs to be validated in order to convince the researchers. Secondly, for films of different eras, outliers mostly appeared before 1950. Whether the rating of that era is valid needs to be explored, because 70 years have passed, and cultural differences will affect the rating level. However, if only comments from 1950 are used as a rating standard, there is a lack of real and effective data on comments. So I think we should do a weighted analysis of films from different eras. The budget of a movie, the length of a movie, the outliers are the same as the chronologies. They should be weighted for different kinds of films, different generations of films. Because the average budget required for different types of films is not consistent. For example, an action blockbuster requires a large budget for setting, special effects, and action, while a comedy usually doesn't. And the budget distribution of the same kind of film varies greatly from generation to generation.
